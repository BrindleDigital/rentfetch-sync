 ...
 * @return  void.
 */
function rfs_realpage_update_unit_from_list_meta( $args, $unit ) {
	...
	$meta = array(
		'availability_date' => esc_html( $formatted_date ),
 ...
 * @return  void.
 */
function rfs_realpage_update_unit_by_property_meta( $args, $unit ) {
	...
	$meta = array(
		'availability_date' => esc_html( $formatted_date ),
		// 'available'         => esc_html( $unit['AvailableBit'] ), // we're only adding the available ones.
		'updated'           => current_time( 'mysql' ),
		// 'beds'              => floatval( $unit['UnitDetails']['Bedrooms'] ),
		// 'baths'             => floatval( $unit['UnitDetails']['Bathrooms'] ),
		'maximum_rent'      => floatval( $maximum_rent_base_rent ),
		'minimum_rent'      => floatval( $maximum_rent_base_rent ),
		'deposit'           => floatval( $unit['DepositAmount'] ),
		'sqrft'             => floatval( $unit['RentSqFtCount'] ),
		'floor_number'      => floatval( $unit['FloorNumber'] ),
		'api_response'      => $api_response,
		'unit_source'       => 'realpage',
	);
 ...
 *
 * @param array $args the arguments that we passed to the function.
