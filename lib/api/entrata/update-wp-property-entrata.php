...
function rfs_entrata_update_property_mits_meta( $args, $property_data ) {
	...
	$meta = [
		...
		// 'url' => esc_url( $data['webSite'] ),
		// 'description' => esc_attr( $data['LongDescription'] ),
		'phone' => esc_html( $data['PropertyID']['Phone'][0]['PhoneNumber'] ), 
		'latitude' => esc_html( $data['ILS_Identification']['Latitude'] ),
		'longitude' => esc_html( $data['ILS_Identification']['Longitude'] ),
		'synced_property_images' => $data['File'],
		// 'updated' => current_time('mysql'),
		'api_response' => $api_response,
	];
